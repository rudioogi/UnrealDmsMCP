...
- **Commands hang for ~30s–5min then succeed on retry** (incl. trivial ones like `get_actors_in_level`, or a tool returning "result missing"): The editor was throttling its tick because its window wasn't focused (you were in the terminal/Claude Code). Every command runs on the game thread, so a throttled editor stalls them. The plugin disables this on startup — both **Editor Preferences → General → Performance → "Use Less CPU when in Background"** and the `Slate.bAllowThrottling` CVar (look for the `EpicUnrealMCPBridge: Disabled 'Use Less CPU when in Background'` log line). Quick manual fix without a rebuild: uncheck that preference (takes effect instantly) and keep the editor window **visible, not minimized**.
